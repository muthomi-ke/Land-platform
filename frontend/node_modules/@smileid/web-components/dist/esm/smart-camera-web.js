...
import { S as i, p as n } from "./package-B-UwEdv7.js";
import "./DocumentCaptureScreens-C5BhNB-0.js";
import "./SelfieCaptureScreens-KoQpCxtc.js";
...
class h extends HTMLElement {
  connectedCallback() {
    this.templateString = o.bind(this), this.render = () => this.templateString(), this.attachShadow({ mode: "open" }), this.shadowRoot.innerHTML = this.render(), this.setUpEventListeners();
  }
  setUpEventListeners() {
...
window.customElements && !window.customElements.get("camera-permission") && window.customElements.define("camera-permission", h);
const c = n.version;
function d() {
  return `
  <style>
...
class l extends HTMLElement {
  constructor() {
    super(), this.scwTemplateString = d.bind(this), this.render = () => this.scwTemplateString(), this.attachShadow({ mode: "open" }), this.activeScreen = null;
  }
  connectedCallback() {
  ...
  setUpEventListeners() {
    this.cameraPermission = this.shadowRoot.querySelector("camera-permission"), this.SelfieCaptureScreens = this.shadowRoot.querySelector(
      "selfie-capture-screens"
    ), this.documentCapture = this.shadowRoot.querySelector(
      "document-capture-screens"
    ), this.hideInstructions ? this.setActiveScreen(this.cameraPermission) : this.setActiveScreen(this.SelfieCaptureScreens), this.cameraPermission.addEventListener("camera-permission.granted", () => {
      this.setActiveScreen(this.SelfieCaptureScreens), this.SelfieCaptureScreens.removeAttribute("data-camera-error"), this.SelfieCaptureScreens.setAttribute("data-camera-ready", !0);
    }), this.SelfieCaptureScreens.addEventListener(
      "selfie-capture-screens.publish",
      (e) => {
        this._data.images = e.detail.images, this.captureId ? this.setActiveScreen(this.documentCapture) : this._publishSelectedImages();
      }
    ), this.SelfieCaptureScreens.addEventListener(
      "selfie-capture-screens.cancelled",
      () => {
        this.hideInstructions ? this.setActiveScreen(this.cameraPermission) : this.handleBackEvents();
      }
    ), this.SelfieCaptureScreens.addEventListener(
      "selfie-capture-screens.back",
      () => {
        this.hideInstructions || this.setActiveScreen(this.cameraPermission);
      }
    ), this.documentCapture.addEventListener(
      "document-capture-screens.publish",
      (e) => {
        this._data.images = [...this._data.images, ...e.detail.images], this._publishSelectedImages();
      }
    ), this.documentCapture.addEventListener(
      ...
      e.addEventListener(
        `${e.nodeName.toLowerCase()}.close`,
        () => this.handleCloseEvent()
      );
    }), this.documentCapture.addEventListener(
      "document-capture-screens.back",
      () => {
        this.setActiveScreen(this.SelfieCaptureScreens), this.reset();
      }
    );
  }
  reset() {
...
"customElements" in window && !customElements.get("smart-camera-web") && customElements.define("smart-camera-web", l);
export {
  l as default
};
